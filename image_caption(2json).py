...
class Vocabulary(object):
    def __init__(self,
                 ...
                 end_word="<end>",
                 unk_word="<unk>",
                 annotations_file='D:/gwangho_pj1_data/merge/annotations/captions_val2017.json',
                 annotations_file2='D:/gwangho_pj1_data/merge/annotations/captions.json',
    ...
    def add_captions(self):
        ...
        coco2 = COCO(self.annotations_file2)
        counter = Counter()  # 빈도수 체크를 위한 counter
        ids = list(set(list(coco.anns.keys()) + list(coco2.anns.keys())))  # 캡션 id목록 생성

        for i, id in enumerate(ids):  # id목록을 반복하면서
            if i % 100000 == 0:
                print("[%d/%d] Tokenizing captions..." % (i, len(ids)))
            if id in coco.anns:  # 캡션 id가 coco1 데이터셋에 존재한다면
                caption = str(coco.anns[id]['caption'])  # 해당 캡션을 갖고온다.
            else:  # coco2 데이터셋이라면
                caption = str(coco2.anns[id]['caption'])  # 해당 캡션 갖고오기

            tokens = nltk.tokenize.word_tokenize(caption.lower())  # 소문자로 변환 후 토큰화 하여 단어 토큰 리스트 생성
            counter.update(tokens)  # 단어의 등장 빈도 업데이트
...
def get_loader(transform,
    ...
    if mode == 'train':
        ...
        img_folder = os.path.join('D:/gwangho_pj1_data/merge/ffdata/val2017')                           # 이미지 폴더 경로
        annotations_file = os.path.join('D:/gwangho_pj1_data/merge/annotations/captions_val2017.json')  # 어노테이션1 경로
        annotations_file2 = None
        #annotations_file2 = os.path.join('D:/gwangho_pj1_data/merge/annotations/captions.json')         # 어노테이션2 경로
    if mode == 'test':   # test 모드일 경우
        assert batch_size == 1, "Please change batch_size to 1 if testing your model."
        assert os.path.exists(vocab_file), "Must first generate vocab.pkl from training data."
        assert vocab_from_file == True, "Change vocab_from_file to True."

        img_folder = os.path.join('D:/gwangho_pj1_data/merge/ffdata')                      # 이미지 경로
        annotations_file = os.path.join('D:/gwangho_pj1_data/merge/annotations/test.json') # 어노테이션 경로
        annotations_file2 = None
    ...
    dataset = CoCoDataset(transform=transform,
                          ...
                          annotations_file2=annotations_file2,
...
class CoCoDataset(data.Dataset):
    def __init__(self, transform, mode, batch_size, vocab_threshold, vocab_file, start_word,
                 end_word, unk_word, annotations_file, annotations_file2, vocab_from_file, img_folder):
        self.transform = transform
        self.mode = mode
        self.batch_size = batch_size
        self.vocab = Vocabulary(vocab_threshold, vocab_file, start_word,
                                end_word, unk_word, annotations_file, annotations_file2, vocab_from_file)
        self.img_folder = img_folder
        if self.mode == 'train':
            self.coco = COCO(annotations_file)
            self.coco2 = COCO(annotations_file2)

            ids_annotation1 = list(self.coco.anns.keys())
            selected_ids = random.sample(ids_annotation1, min(len(ids_annotation1), 500))

            ids_annotation2 = list(self.coco2.anns.keys())
            selected_ids2 = random.sample(ids_annotation2, min(len(ids_annotation2), 500))

            self.ids = list(set(selected_ids + selected_ids2))

            print('Obtaining caption lengths...')
            all_tokens = []
            for index in tqdm(np.arange(len(self.ids))):
                caption1 = self.coco.anns.get(self.ids[index], {}).get('caption', '')
                caption2 = self.coco2.anns.get(self.ids[index], {}).get('caption', '')
                tokens1 = nltk.tokenize.word_tokenize(str(caption1).lower())
                tokens2 = nltk.tokenize.word_tokenize(str(caption2).lower())
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            if ann_id in self.coco.anns:
                coco_anns = self.coco.anns
                coco_img_id = self.coco.anns[ann_id]['image_id']
                coco_dataset = self.coco
            else:
                coco_anns = self.coco2.anns
                coco_img_id = self.coco2.anns[ann_id]['image_id']
                coco_dataset = self.coco2

            path = coco_dataset.loadImgs(coco_img_id)[0]['file_name']
            ...
            caption = coco_anns[ann_id]['caption']
...
# val_data_loader = torch.utils.data.DataLoader(val_dataset, batch_size=val_batch_size, shuffle=True)
#
# print("Training dataset size:", len(train_data_loader.dataset))
# print("Validation dataset size:", len(val_data_loader.dataset))
...
split_index = int(0.9 * len(data_loader.dataset))
train_indices = indices[:split_index]
val_indices = indices[split_index:]

train_data_loader = torch.utils.data.DataLoader(dataset=data_loader.dataset,
                                                batch_size=batch_size,
                                                sampler=torch.utils.data.sampler.SubsetRandomSampler(train_indices))
val_data_loader = torch.utils.data.DataLoader(dataset=data_loader.dataset,
                                              batch_size=batch_size,
...
total_step = math.ceil(len(data_loader.dataset.caption_lengths) / batch_size)
